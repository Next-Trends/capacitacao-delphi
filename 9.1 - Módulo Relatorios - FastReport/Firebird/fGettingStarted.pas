...
uses
  ...
  FireDAC.Phys.IBWrapper, frxClass, frxDBSet, frCoreClasses;
...
begin
  // Update records
  dbMain.ExecSQL('update PRODUTOS set UnitPrice = UnitPrice * :P1 + :P2 ' +
                 'where ID < 3', [Random(5), Random(3)]);
  qryProdutos.Refresh;

  // Get a scalar value from DB
  dMaxPrice := dbMain.ExecSQLScalar('select MAX(VALOR) from PRODUTOS');
  sbMain.SimpleText := Format('Maior Valor = %m', [dMaxPrice]);
end;
